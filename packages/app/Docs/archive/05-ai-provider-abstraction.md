...
**Canonical scheme:** Use only `AI_PROVIDER`, `AI_API_KEY`, and `AI_MODEL` for AI configuration. Do not use provider-specific names like `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` in your setup—the app reads a single `AI_API_KEY` and passes it to the selected provider. Provider-specific variable names are implementation details only (e.g. inside a provider SDK that expects a certain name).

The project’s `.env.example` at the repo root is the canonical reference; copy it to `.env.local` and fill in your key.

### Required Environment Variables

```bash
# .env.local (match .env.example in repo)

# AI provider (canonical)
AI_PROVIDER=anthropic  # anthropic | openai | google
AI_API_KEY=            # Your API key for the chosen provider (leave blank in .env.example)
...
Use the **same** `AI_API_KEY` variable; only the value and `AI_PROVIDER` / `AI_MODEL` change:
...
# OpenAI (same AI_API_KEY variable, different value)
AI_PROVIDER=openai
AI_API_KEY=sk-...
AI_MODEL=gpt-4o

# Google (same AI_API_KEY variable, different value)
AI_PROVIDER=google
AI_API_KEY=...
...
describe('AI Provider Comparison', () => {
  ...
  test('OpenAI generates valid content', async () => {
    // Use canonical AI_API_KEY; when AI_PROVIDER=openai this is your OpenAI key
    const provider = createProvider('openai', process.env.AI_API_KEY!)
    const result = await provider.generateContent(testPrompt, {
      model: 'gpt-4o',
      maxTokens: 500,
      temperature: 1
