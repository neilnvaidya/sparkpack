...
The selected AI provider is configured via environment variables in `.env.local`. **Canonical variables** (use only these; no provider-specific names like `ANTHROPIC_API_KEY`):

```bash
AI_PROVIDER=anthropic  # or openai, google
AI_API_KEY=sk-...      # API key for the chosen provider
AI_MODEL=claude-sonnet-4-20250514   # Optional: override default
NEXT_PUBLIC_APP_NAME="SparkPack"
NEXT_PUBLIC_MAX_GAMES_PER_DAY=10
```

Copy `.env.example` from the repo root to `.env.local` and fill in your key.
...
Use the **canonical** scheme only: `AI_PROVIDER`, `AI_API_KEY`, `AI_MODEL`. The project’s `.env.example` is the source of truth; copy to `.env.local` and add your key.

Required in `.env.local` (must match `.env.example`):

```bash
# AI provider (canonical; no ANTHROPIC_API_KEY / OPENAI_API_KEY etc.)
AI_PROVIDER=anthropic
AI_API_KEY=                            # Fill in; leave blank in .env.example
...
Optional (not in `.env.example`): `LOG_LEVEL`, `ENABLE_DEBUG_LOGGING`, `SKIP_VALIDATION`.
...
### API Key Protection
- Never expose `AI_API_KEY` to client (canonical name for the chosen provider’s key)
