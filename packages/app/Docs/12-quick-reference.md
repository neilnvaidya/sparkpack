...
**.env.local** (copy from `.env.example`, add your key; never commit):
```bash
# Canonical scheme only (no ANTHROPIC_API_KEY / OPENAI_API_KEY)
AI_PROVIDER=anthropic
AI_API_KEY=sk-...               # Key for chosen provider
