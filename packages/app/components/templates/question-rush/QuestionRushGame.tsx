...
import { useGameStore, type QuestionRushState } from '@/lib/store/game-store'
import type { QuestionRushQuestion } from '@/lib/question-rush/question'
import { buildAnswerLine } from '@/lib/question-rush/question'
import { cn } from '@/lib/utils/cn'
import { getTeamColorDef, CARD_COLOR_OPTIONS, type ColorDef } from '@/lib/constants/team-colors'
import { useSoundStore } from '@/lib/store/sound-store'
import type { QuestionRushContent } from '@/lib/question-rush/content'
...
const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  { target: 'cards', title: 'A team calls it', body: 'Each card has a colour. A team calls out the colour and its answer — tap that card to select it.' },
...
function QuestionRushCard({
  ...
  return (
    <div
      ...
      className={cn(
        'relative flex min-h-[200px] flex-col rounded-[var(--radius-xl)] border-2 bg-surface p-4 transition-all',
      ...
      // Unclaimed cards wear their own colour so a team can name one out loud;
      // once claimed, the owning team's colour takes over.
      style={{ borderColor: claimedTeam && claimColor ? claimColor.hex : cardColor.hex }}
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <span
          className="flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-0.5 font-display text-sm font-extrabold text-white"
          style={{ backgroundColor: cardColor.hex }}
        >
          {cardColor.name}
        </span>
        <span className="sbq-point-badge shrink-0 px-2.5 py-0.5 font-display text-xs font-extrabold">
          {points} pts
        </span>
      </div>
      ...
      {claimedTeam && claimColor && (
        <span
          className="mb-2 flex max-w-full items-center gap-1.5 self-start rounded-full border bg-surface-alt px-2 py-0.5 text-xs font-semibold text-text-primary"
      ...
      {answerRevealed && (
        <div className="mt-3 border-t border-border pt-3 text-center">
          ...
          <div className="font-mono text-base font-bold text-[var(--color-correct)]">
...
export default function QuestionRushGame() {
  const phase = useGameStore((s) => s.phase)
  const teams = useGameStore((s) => s.teams)
  const content = useGameStore((s) => s.content) as QuestionRushContent | null
  const templateState = useGameStore((s) => s.templateState as QuestionRushState | null)
  const endGame = useGameStore((s) => s.endGame)
  const questionRushOpenCard = useGameStore((s) => s.questionRushOpenCard)
  const questionRushAwardCard = useGameStore((s) => s.questionRushAwardCard)
  const questionRushRevealCard = useGameStore((s) => s.questionRushRevealCard)
  const questionRushRevealAllAnswers = useGameStore((s) => s.questionRushRevealAllAnswers)
  const questionRushNextRound = useGameStore((s) => s.questionRushNextRound)
  const questionRushClearLastAward = useGameStore((s) => s.questionRushClearLastAward)
  const playSound = useSoundStore((s) => s.play)

  const gameTitle = content?.title ?? 'Question Rush'
  const lastAward = templateState?.lastAward ?? null
  const openCardIndex = templateState?.openCardIndex ?? null

  useEffect(() => {
    if (!lastAward) return
    const t = window.setTimeout(() => questionRushClearLastAward(), 2800)
    return () => window.clearTimeout(t)
  }, [lastAward, questionRushClearLastAward])

  // Escape clears the selected card.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') questionRushOpenCard(null)
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [questionRushOpenCard])
  ...
  const hint =
    openCardIndex !== null
      ...
      : allClaimed
        ...
        : 'When a team calls out a colour and its answer, tap that card.'
  ...
  const actions: ShellAction[] = [
    {
      ...
      variant: 'neutral',
      onClick: () => questionRushRevealAllAnswers(),
    },
    {
      id: 'next',
      label: isFinalRound ? 'Finish' : 'Next round',
      variant: 'primary',
      onClick: () => questionRushNextRound(),
    },
    ...
    <GameShell
      ...
      teamsPanel={{
        ...
        onTeamClick: (ti) => {
          ...
          questionRushAwardCard(openCardIndex, ti)
          playSound('correct')
        },
        teams: teams.map((t) => ({ name: t.name, colorId: t.color, score: t.score })),
      }}
      tutorial={{ id: 'question_rush', steps: TUTORIAL_STEPS }}
    >
      <div className="flex h-full w-full flex-col gap-3">
        ...
        {templateState && (
          <div
            data-tutorial="cards"
            className="mx-auto grid w-full flex-1 content-center gap-4 overflow-auto"
            style={{
              // One column per card: with only three, auto-fill left them narrow
              // and stranded at one side of a projector.
              gridTemplateColumns: `repeat(${Math.max(1, templateState.activeCards.length)}, minmax(0, 1fr))`,
              maxWidth: 'min(100%, 1100px)',
            }}
          >
            {templateState.activeCards.map((card, i) => (
              <QuestionRushCard
                key={`${card.question.id ?? card.question.prompt}-${i}`}
                question={card.question}
                points={card.question.points}
                cardColor={CARD_COLOR_OPTIONS[i % CARD_COLOR_OPTIONS.length]}
                claimedByTeamIndex={card.claimedByTeamIndex}
                answerRevealed={card.answerRevealed}
                teams={teams}
                isOpen={openCardIndex === i}
                onCardClick={() => questionRushOpenCard(openCardIndex === i ? null : i)}
                onRevealToggle={() => questionRushRevealCard(i)}
              />
            ))}
          </div>
        )}
